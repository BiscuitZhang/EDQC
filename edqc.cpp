...
std::vector<int> extract_QC(const std::vector<double> &energy, const std::vector<int> &active_nodes, const Graph &G, double density_threshold)
{
    ...
    for (const auto &pe : P)
    {
        ...
        if (current_vol > 0 && total_vol - current_vol > 0)
        {
            ...
            if (current_S.size() >= 3 && conductance < min_conductance)
            {
    ...
    if (S_initial.empty())
    {
        if (P.size() >= 3)
        {
            for (size_t i = 0; i < std::min((size_t)P.size(), (size_t)3); ++i)
    ...
    while ((int)S.size() > 3 && !DensityOK((int)S.size(), eS, density_threshold))
    {
        int min_pos = 0;
    ...
    if (!DensityOK((int)S.size(), eS, density_threshold) && S.size() > 0)
        return S; 
    if (S.empty())
        return {};
