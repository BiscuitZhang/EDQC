...
int searchInAdd(int &v_aim,int iter)
{
    int u, v, w=-1, count;
    bool no_cc = false;
    temp_array_size = 0;
    ...
    if(count == u + cur_sol_size)
    {
        if(w != -1) {  // 确保w已被初始化
            ...
            // 如果w未初始化，采取备选方案
            if(cur_sol_size > 0) {
                v = cur_sol[rand() % cur_sol_size];  // 随机选一个节点
                no_cc = true;
            }
            else {
                // 处理极端情况：当前解为空
    ...
    v_aim = randInTieMax(temp_array, temp_array_size, profit, populationHistory[iter]);
    //v_aim = randInTieMax(temp_array, temp_array_size, profit, NULL);
    return 0;
}

int searchInDrop(int &u_aim, int tabu_v,int iter)
{
    int tmp1 = 0, tmp2 = 0;
    if(Density > 0.5)
    {
        u_aim = randInTieMin(cur_sol, cur_sol_size - 1, profit, populationHistory[iter]);
    ...
    u_aim = randInTieMin(temp_array, temp_array_size, profit, populationHistory[iter]);
    //u_aim = randInTieMin(temp_array, temp_array_size, profit, NULL);
    return 0;
}

int imformationFeedback(int iter,int v_aim) {
    r1 = rand() % v_num;
    f_t_minus_1 = scoreHistory[iter-1][r1];
    F_t_plus_1 = scoreHistory[iter][v_aim];
    alpha = f_t_minus_1 / (f_t_minus_1 + F_t_plus_1);
    beta = F_t_plus_1 / (f_t_minus_1 + F_t_plus_1);
    int ret = alpha * populationHistory[iter][v_aim] + beta * populationHistory[iter - 1][r1];
    return ret;
}
...
int searchQuasi()
{
    ...
    while(getUtime() <= Cuff_time)
    {
        if(initSolution() != 0)
            return 1;
        no_expand_iter = 0;
        edge_in_complete = cur_sol_size * (cur_sol_size - 1) >> 1;//当前选择顶点可构成的最大边数
        edge_in_quasi = ceil(double(edge_in_complete) * param_k);//可构成的符合要求密度的伽马准团所需最少顶点数
        while(no_expand_iter < Max_iter)
        {
            if(cur_sol_quality >= edge_in_quasi)
            {
                ...
                rn = searchInAdd(v_aim, no_expand_iter);
                addVertex(v_aim);
                /*imformationfeedback*/
                populationHistory[no_expand_iter][v_aim] = cur_sol_size;//当前顶点的适应度值初始化
                if (no_expand_iter >= 3) {
                    populationHistory[no_expand_iter][v_aim] = imformationFeedback(no_expand_iter,v_aim);//根据历史信息重新计算当前顶点的适应度值
                }
                /**/
                subscore[v_aim] = 0;
                scoreHistory[no_expand_iter][v_aim] = 0;
                tabu_v = v_aim;
                no_expand_iter = 0;
                continue;
            }
            ...
            scoreHistory[no_expand_iter][v_aim] = 0;
            tabu_v = v_aim;
            rn = searchInDrop(u_aim, tabu_v, no_expand_iter);
            removeVertex(u_aim);
            /*imformationfeedback*/
            populationHistory[no_expand_iter][u_aim] = 0;//当前顶点的适应度值初始化
            if (no_expand_iter >= 3) {
                populationHistory[no_expand_iter][u_aim] = imformationFeedback(no_expand_iter,u_aim);//根据历史信息重新计算当前顶点的适应度值
            }
            /**/
            subscore[u_aim] = 0;
            scoreHistory[no_expand_iter][u_aim] = 0;
            for(int i = 0; i < neighbor_s_size; ++i)
            {
                u = neighbor_s[i];
                subscore[u] += profit[u];
                scoreHistory[no_expand_iter][u] = subscore[u];
            }
            for(int i = 0; i < cur_sol_size; ++i)
            {
                u = cur_sol[i];
